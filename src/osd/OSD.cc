...
bool OSDService::_get_map_bl(epoch_t e, bufferlist& bl)
{
  ...
  if (found)
    return true;
  found = store->read(coll_t(), OSD::get_osdmap_pobject_name(e), 0, 0, bl) >= 0;
  if (found)
    _add_map_bl(e, bl);
  return found;
}
...
bool OSDService::get_inc_map_bl(epoch_t e, bufferlist& bl)
{
  ...
  if (found)
    return true;
  found = store->read(
    coll_t(), OSD::get_inc_osdmap_pobject_name(e), 0, 0, bl) >= 0;
  if (found)
	      ...
	      uuid_d fsid, int whoami)
{
  ...
  try {
    ...
    OSDSuperblock sb;
    bufferlist sbbl;
    ret = store->read(coll_t(), OSD_SUPERBLOCK_POBJECT, 0, 0, sbbl);
    if (ret >= 0) {
	...
	for (int i=0; i<1000; i++) {
	  ...
	  t->write(coll_t(), hobject_t(sobject_t(oid, 0)), i*bl.length(), bl.length(), bl);
	...
	tr.remove(coll_t(), hobject_t(sobject_t(oid, 0)));
	...
	sb.weight = (1000.0 / (double)end);
      ...
      t.write(coll_t(), OSD_SUPERBLOCK_POBJECT, 0, bl.length(), bl);
...
int OSD::init()
{
  ...
  if (!store->exists(coll_t(), OSD::make_snapmapper_oid())) {
    dout(10) << "init creating/touching snapmapper object" << dendl;
    ObjectStore::Transaction t;
    t.touch(coll_t(), OSD::make_snapmapper_oid());
    r = store->apply_transaction(t);
    if (r < 0)
      goto out;
  }
...
{
  bufferlist bl;
  int r = store->read(coll_t(), OSD_SUPERBLOCK_POBJECT, 0, 0, bl);
  if (r < 0)
    return r;
...
void OSD::load_pgs()
{
  ...
  if (has_upgraded && store->exists(coll_t(), OSD::make_infos_oid())) {
    dout(1) << __func__ << " removing legacy infos object" << dendl;
    ObjectStore::Transaction t;
    t.remove(coll_t(), OSD::make_infos_oid());
    int r = store->apply_transaction(t);
    if (r != 0) {
...
void OSD::do_command(Connection *con, ceph_tid_t tid, vector<string>& cmd, bufferlist& data)
{
  ...
  // 'tell <pgid>' (which comes in without any of that prefix)?
    ...
    for (int64_t pos = 0; pos < count; pos += bsize) {
      ...
      ObjectStore::Transaction *t = new ObjectStore::Transaction;
      t->write(coll_t(), soid, 0, bsize, bl);
      store->queue_transaction_and_cleanup(NULL, t);
      cleanupt->remove(coll_t(), soid);
    }
    store->sync_and_flush();
    utime_t end = ceph_clock_now(cct);
...
void OSD::handle_osd_map(MOSDMap *m)
{
  ...
  for (epoch_t e = start; e <= last; e++) {
	...
	last_marked_full = e;
      ...
      t.write(coll_t(), fulloid, 0, bl.length(), bl);
      pin_map_bl(e, bl);
    ...
    if (p != m->incremental_maps.end()) {
      ...
      t.write(coll_t(), oid, 0, bl.length(), bl);
      pin_map_inc_bl(e, bl);
	...
	break;
      ...
      hobject_t fulloid = get_osdmap_pobject_name(e);
      t.write(coll_t(), fulloid, 0, fbl.length(), fbl);
      pin_map_bl(e, fbl);
      pinned_maps.push_back(add_map(o));
      continue;
    }
  ...
  if (superblock.oldest_map) {
    ...
    for (epoch_t e = superblock.oldest_map; e < min; ++e) {
      ...
      t.remove(coll_t(), get_osdmap_pobject_name(e));
      t.remove(coll_t(), get_inc_osdmap_pobject_name(e));
